...
namespace GME1003GoblinDanceParty
{
    public class Game1 : Game
    {
        ...
        private List<float> _RotationList;
        private List<float> _TransparencyList;
        private List<float> _SizeList;
        private List<Color> _ColorList;

        private Texture2D _starSprite;  //the sprite image for our star or fry guy
        ...
        protected override void Initialize()
        {
            ...
            _numStars = _rng.Next(100, 301);              //this would be better as a random number between 100 and 300
            _starsX = new List<int>();  //stars X coordinate
            _starsY = new List<int>();  //stars Y coordinate
            _RotationList = new List<float>();
            _TransparencyList = new List<float>();
            _SizeList = new List<float>();
            ...
            for (int i = 0; i < _numStars; i++)
            {
                _RotationList.Add(_rng.Next(0, 101) / 100f); //all star rotation values are random
            }

            for (int i = 0; i < _numStars; i++)
            {
                _TransparencyList.Add(_rng.Next(25, 101) / 100f); //all stars have random transparency
            }

            for (int i = 0; i < _numStars; i++)
            {
                _SizeList.Add(_rng.Next(50, 100) / 200f); //all stars have random size
            }

            for (int i = 0; i < _numStars; i++)
            {
                _ColorList.Add(new Color(128 + _rng.Next(0, 129), 128 + _rng.Next(0, 129), 128 + _rng.Next(0, 129))); //all stars have random colors
            }
        ...
        protected override void LoadContent()
        {
            ...
            _starSprite = Content.Load<Texture2D>("fry fighter");
            _background = Content.Load<Texture2D>("hampter");
        ...
        //you could make that happen with a single Draw statement.
            ...
            for (int i = 0; i < _numStars; i++) 
            {
                _spriteBatch.Draw(_starSprite, 
                    ...
                    null,                                   //ignore this
                    _ColorList[i] * _TransparencyList[i],         //set colour and transparency
                    _RotationList[i],                          //set rotation
                    new Vector2(_starSprite.Width / 2, _starSprite.Height / 2), //ignore this
                    new Vector2(_SizeList[i], _SizeList[i]),    //set scale (same number 2x)
